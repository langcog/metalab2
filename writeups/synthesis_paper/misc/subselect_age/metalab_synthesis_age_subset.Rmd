...
### READ IN AGE SUBSETTED DATA
all_data <- read.csv("all_data_age_subset.csv")
...
funnel.es.data = all_data %>%
  mutate(dataset = as.factor(dataset),
         dataset= gdata::reorder.factor(dataset, 
                                    new.order = c(2,6,10,11,9,4,8,3,5,1,7)),
...
table.data = left_join(select(all_ds, c(short_name, d_string)), fsn.package.data) %>%
             ...
             ungroup() %>%
             .[c(2,8,3,4,10,5,7,11,6,1,9),]  %>% # reorder rows 
             mutate(dataset = as.factor(dataset),
...
```{r, level.plot.data}
# make levels df
ld.df = data.frame(dataset = datasets$name[-6],
                   domain = c("Prosody", "Words", "Communication", "Sounds",
                              "Sounds",  "Sounds", "Sounds", "Words",
                              "Words", "Words"))
...
real.levels = levels(droplevels(simulated.development.all$dataset))
fake.levels = c("1", "2", "3") 
...
synthesis.plot = ggplot(simulated.development.all,
  ...
  scale_colour_manual(values=c( "grey70", "grey35", "grey20", "#d33682", 
                                ...
                                "#268bd2" ,"#6c71c4", "#993399" ,"#268bd2","#268bd2"),
...
legend.desc2 = simulated.development.all %>%
  ...
  dataset.label = gdata::reorder.factor(dataset.label, 
                                    new.order = rev(c(6,5,4,9,7,8,2,1,3))))%>%
...
r_refs(file = "../NHB/metalab_synthesis.bib")
```
